...
3. Find out how many players were drafted in these ranges that just straight up never appeared in a game. Want to take those guys, add 0s into the df again to make my rookie projection not suffer from selection bias as much
  i. Probably going to need to use the same estimation techniques for these too. Sample 10 players in the bucket of never         appeared in the game, use that to estimate how many 0s to add for each guy (don't wanna just add 17 for each one)
```{r, message=FALSE}
library(tidyverse)
library(dplyr)
library(rsconnect)
...
I should move this to the .Rmd where I actually make this at some point, not a super big problem rn but def should do it in the future.

Started Google sheet with temp1
5/15
Definitely going to need to random sample. This isn't worth doing all by hand to figure out exact numbers, I'm ok if there's flaws, but there needs to be zeros added. I think the buckets under 10-15 I need to do all of them (shouldn't be as bad). Everyone else, random sample 10-15, throw them in the google doc, use that to estimate how many each player would miss. Still going to have to figure out a way to estimate how many ppl just straight up never played in any games that were in the bucket too. Maybe I do that first? find all the names that should be in the buckets (like 6th round running backs), then randomly sample. I'll get some that miss 0, I'll get some that miss 16 even tho they were healthy, should even out if I sample enough, should be easier. I don't know what the right way to do that is tbh, but I think independent random sampling points to throwing them all in the same pile and picking from there. 
```{r}
...
Have no position or name on a couple of players (40 out of 6085 observations). Probably just gonna toss them out of the sample. 
```{r}
anti_join(Rookie_database, temp, by = c("week", "player_id"))
```
...
Old buckets for Rookie_database 
```{r}
...
```{r}
linear_sim <- function(ff, team1, team2) {
#stores column names with player's ids
player_ids_vector <- c()
all_player_scores_list <- list()
ff <- ff %>%
...
IMPORTANT ^
Good as of 4/10
Need to reconfigure this a lot now with the update to Linear Sim. It's even more crucial that this part runs fast since it's doing even more calculations than the first part is. 
```{r}
player_ids_vector <- c()
all_player_scores_list <- list()
ids <- ff_2 %>%
