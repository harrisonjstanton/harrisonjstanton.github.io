...
3. 4/12 redo log and linear sim to make long_scores_df right away. would make the whole process way more efficient to not be creating 2 massive dfs when I could just do 1 
4. 4/12 filter out before week 12 in Rookie database when I'm doing my bucket calculations. Why am I using jamar chase's playoff scores? with late rookie breakout/stuff like that, I think it might be wise to just use the weeks that I'm looking at projecting for an individual player. 
```{r, message=FALSE}
library(tidyverse)
library(dplyr)
library(rsconnect)
